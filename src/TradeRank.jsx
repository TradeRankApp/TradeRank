...
function Input({label,type="text",value,onChange,placeholder,maxLength,multiline=false,rows=3,hint=null}){
  ...
  return(<div style={{marginBottom:14}}>
    ...
    {multiline?<textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows} onFocus={()=>setFoc(true)} onBlur={()=>setFoc(false)} style={{...s,color:'#fff'}}/>
    :<input type={type} value={value} onChange={onChange} placeholder={placeholder} maxLength={maxLength} onFocus={()=>setFoc(true)} onBlur={()=>setFoc(false)} style={{...s,color:'#fff'}}/>}
...
function EmptyState({emoji,title,subtitle,action,actionLabel}){
  return(<div className="fade-up" style={{padding:"60px 32px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
    ...
    <div style={{fontSize:17,fontWeight:700,color:"#fff",letterSpacing:"-0.02em",marginBottom:8,lineHeight:1.3,fontFamily:"'Syne',sans-serif"}}>{title}</div>
...
function VerifyBanner({onVerify}){const [v,setV]=useState(true);if(!v)return null;return(<div className="fade-up" style={{margin:"12px 16px",borderRadius:16,background:G.greenLight,border:`1px solid ${G.greenBorder}`,padding:"14px 16px",position:"relative"}}><button onClick={()=>setV(false)} style={{position:"absolute",top:10,right:10,background:"none",border:"none",fontSize:16,color:G.g400,cursor:"pointer"}}>×</button><div style={{display:"flex",gap:12,alignItems:"center"}}><div style={{width:44,height:44,borderRadius:"50%",background:G.green,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0,boxShadow:`0 4px 12px ${G.green}50`}}>✓</div><div style={{flex:1,paddingRight:16}}><div style={{fontSize:14,fontWeight:800,color:"#fff",marginBottom:3}}>Get your green checkmark</div><div style={{fontSize:12,color:G.g600,lineHeight:1.5,marginBottom:10}}>Upload your trade history — earn a verified score and unlock strategy sales.</div><Btn size="sm" onClick={onVerify}>Verify my trades</Btn></div></div></div>);}
    ...
    <div style={{display:"flex",gap:10,overflowX:"auto",padding:"0 16px"}}>
      {trending.map(p=>{const trader=traders.find(t=>t.id===p.traderId);if(!trader)return null;const isTrade=p.type==="trade";const green=isTrade&&p.pnl>=0;
        return(<div key={p.id} onClick={()=>onTraderClick(trader)} className="press" style={{flexShrink:0,width:155,background:G.bg2,borderRadius:14,padding:12,cursor:"pointer",border:`1px solid ${G.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><Avatar name={trader.name} size={24} verified={trader.verified}/><span style={{fontSize:11,fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{trader.name.toLowerCase()}</span></div>
          ...
          :<p style={{fontSize:12,color:"#fff",lineHeight:1.5,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:3,WebkitBoxOrient:"vertical"}}>{p.text}</p>}
...
function LeaveReviewModal({strategy,onClose,onSubmit}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1002,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
    <div className="slide-up" style={{background:G.bg2,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,margin:"0 auto",padding:"0 20px 36px"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{fontSize:18,fontWeight:800,color:"#fff",marginBottom:4}}>Leave a review</div>
  ...
  if(buyStep==="success")return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1001,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div className="bounce-in" style={{background:G.bg2,borderRadius:22,padding:36,border:`1px solid ${G.border}`,width:"100%",maxWidth:340,textAlign:"center"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",marginBottom:8}}>Strategy Unlocked!</div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1001,display:"flex",alignItems:"flex-end"}} onClick={()=>setBuyStep(null)}>
      <div className="slide-up" style={{background:G.bg2,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,margin:"0 auto",padding:"0 20px 36px",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
          ...
          <div><div style={{fontSize:15,fontWeight:800,color:"#fff"}}>Complete purchase</div><div style={{fontSize:11,color:G.g500}}>{strategy.title} · ${strategy.price}</div></div>
        </div>
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1000,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
    <div className="slide-up" style={{background:G.bg2,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,margin:"0 auto",maxHeight:"93vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{background:G.black,margin:"14px 16px 0",borderRadius:16,padding:"16px 18px"}}>
        ...
        <div style={{fontSize:19,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",lineHeight:1.2,marginBottom:6}}>{strategy.title}</div>
      ...
      <div style={{padding:"16px 18px"}}>
        {tab==="overview"&&(<><p style={{fontSize:14,color:G.g600,lineHeight:1.75,marginBottom:14}}>{strategy.description}</p><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{strategy.tags.map(tag=>(<span key={tag} style={{fontSize:12,fontWeight:600,color:G.g600,background:G.g100,borderRadius:20,padding:"4px 11px"}}>{tag}</span>))}</div></>)}
        {tab==="preview"&&(
          <div>
            <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:12}}>{strategy.preview.title}</div>
            {strategy.preview.points.map((pt,i)=>(<div key={i} style={{display:"flex",gap:10,marginBottom:10,padding:"10px 13px",background:G.g50,borderRadius:12,border:`1px solid ${G.border}`}}><div style={{width:20,height:20,borderRadius:"50%",background:G.green,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,fontWeight:900,color:G.black,marginTop:1}}>✓</div><span style={{fontSize:13,color:"#fff",lineHeight:1.5}}>{pt}</span></div>))}
            <div style={{position:"relative",marginTop:8,borderRadius:14,overflow:"hidden"}}>
              ...
              {!owned&&<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#ffffffcc",borderRadius:14,textAlign:"center",padding:20}}><div style={{fontSize:22,marginBottom:8}}>🔒</div><div style={{fontSize:14,fontWeight:800,color:"#fff",marginBottom:4}}>Purchase to read more</div><div style={{fontSize:12,color:G.g500,marginBottom:12}}>Full strategy reveals all variations, charts, and rules.</div><Btn size="sm" onClick={()=>setBuyStep("payment")}>Buy for ${strategy.price}</Btn></div>}
            </div>
          </div>
        )}
        {tab==="includes"&&(<div><div style={{fontSize:12,fontWeight:700,color:G.g500,marginBottom:12,letterSpacing:"0.04em"}}>WHAT YOU GET</div>{strategy.includes.map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"flex-start",gap:11,marginBottom:10,padding:"10px 13px",background:G.g50,borderRadius:12,border:`1px solid ${G.border}`}}><div style={{width:22,height:22,borderRadius:"50%",background:G.green,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:11,color:G.black,fontWeight:900}}>✓</div><span style={{fontSize:14,color:"#fff",lineHeight:1.5}}>{item}</span></div>))}</div>)}
        {tab==="reviews"&&(
          <div>
            <div style={{background:G.g50,borderRadius:14,padding:14,marginBottom:14,display:"flex",gap:14,alignItems:"center",border:`1px solid ${G.border}`}}>
              <div style={{textAlign:"center"}}><div style={{fontSize:36,fontWeight:800,color:"#fff",letterSpacing:"-0.04em",lineHeight:1}}>{avgRating.toFixed(1)}</div><StarRating rating={avgRating} size={15}/><div style={{fontSize:11,color:G.g400,marginTop:3}}>{reviews.length} reviews</div></div>
        ...
        <div style={{marginTop:8,paddingTop:14,borderTop:`1px solid ${G.border}`}}>
          {owned?(<div><div style={{background:G.greenLight,borderRadius:13,padding:"12px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:10,border:`1px solid ${G.green}30`}}><span style={{fontSize:20}}>✅</span><div><div style={{fontSize:14,fontWeight:600,color:"#fff"}}>You own this strategy</div><div style={{fontSize:12,color:G.g500}}>Full access unlocked.</div></div></div><Btn full variant="ghost" icon="💬" onClick={()=>onOpenDMs(seller)}>Message {seller?.name?.toLowerCase()}</Btn></div>)
...
function BundleCard({bundle,seller,onBuy,owned}){
  ...
  if(buyStep==="success")return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1002,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={()=>setBuyStep(null)}><div className="bounce-in" style={{background:G.bg2,borderRadius:22,padding:36,border:`1px solid ${G.border}`,width:"100%",maxWidth:320,textAlign:"center"}} onClick={e=>e.stopPropagation()}><div style={{fontSize:48,marginBottom:14}}>🎁</div><div style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:8}}>Bundle Unlocked!</div><p style={{fontSize:13,color:G.g500,lineHeight:1.65,marginBottom:20}}>All strategies in this bundle are now yours forever.</p><Btn full onClick={()=>{onBuy(bundle.id);setBuyStep(null);}}>View strategies</Btn></div></div>);
  if(buyStep==="payment")return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:1002,display:"flex",alignItems:"flex-end"}} onClick={()=>setBuyStep(null)}><div className="slide-up" style={{background:G.bg2,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,margin:"0 auto",padding:"0 20px 36px"}} onClick={e=>e.stopPropagation()}>
    <div style={{width:36,height:4,background:G.border3,borderRadius:2,margin:"14px auto 18px"}}/>
    <div style={{fontSize:16,fontWeight:800,color:"#fff",marginBottom:14}}>{bundle.title}</div>
    <div style={{background:G.greenLight,borderRadius:12,padding:"12px 16px",marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center",border:`1px solid ${G.green}30`}}><div><div style={{fontSize:12,color:G.g500,textDecoration:"line-through"}}>${bundle.originalPrice} separately</div><div style={{fontSize:22,fontWeight:800,color:"#fff"}}>${bundle.price} <span style={{fontSize:13,fontWeight:700,color:G.profit}}>Save ${bundle.savings}</span></div></div><span style={{fontSize:28}}>🎁</span></div>
    <Input label="Name on card" value={name} onChange={e=>setName(e.target.value)} placeholder="Your name"/>
...
function StrategyCard({strategy,seller,onView,animDelay=0}){
  return(<div className="fade-up hov" onClick={()=>onView(strategy)} style={{margin:"0 16px 12px",borderRadius:16,border:`1px solid ${strategy.featured?G.gold+"55":G.g100}`,background:G.bg2,cursor:"pointer",overflow:"hidden",boxShadow:strategy.featured?`0 4px 20px ${G.gold}18`:"0 2px 10px #0000000a",animationDelay:`${animDelay}ms`,transition:"box-shadow 0.2s,transform 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-1px)";}} onMouseLeave={e=>{e.currentTarget.style.transform="none";}}>
    ...
    <div style={{padding:"13px 16px"}}>
      <div style={{fontSize:14,fontWeight:700,color:"#fff",letterSpacing:"-0.02em",marginBottom:3,lineHeight:1.3}}>{strategy.title}</div>
...
function StrategiesView({traders,currentUser,onShowCreate,onOpenDMs}){
  ...
  return(<div>
    ...
    <div style={{padding:"14px 16px 0"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}><h1 style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.03em"}}>Strategies</h1>{currentUser?.verified&&<Btn size="sm" variant="dark" onClick={onShowCreate} icon="📋">Sell yours</Btn>}</div>
    ...
    {!currentUser?.verified&&!currentUser?.isGuest&&<div style={{margin:"0 16px 14px",background:G.greenLight,borderRadius:14,padding:"12px 16px",border:`1px solid ${G.green}30`,display:"flex",gap:10,alignItems:"center"}}><span style={{fontSize:22}}>📈</span><div><div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:2}}>Want to sell your strategy?</div><div style={{fontSize:11,color:G.g600}}>Verify your trades first — buyers trust verified sellers.</div></div></div>}
...
function PostCard({post,trader,onLike,onShare,onComment,onTraderClick,animDelay=0}){
  ...
  return(<div className="fade-up" style={{borderBottom:`1px solid ${G.border}`,paddingTop:16,animationDelay:`${animDelay}ms`,background:G.black}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>onTraderClick(trader)}>
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:"'Syne',sans-serif"}}>{trader.name.toLowerCase()}</span>
      ...
      {isTrade?(
        <div style={{background:G.bg2,borderRadius:14,padding:14,border:`1px solid ${G.border}`}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
            <div>
              <div style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",fontFamily:"'Syne',sans-serif",lineHeight:1}}>{post.symbol}</div>
    ...
    {showC&&(<div className="fade-in" style={{padding:"10px 16px",borderTop:`1px solid ${G.border}`,background:G.bg2}}>
      {post.comments.map(c=>(<div key={c.id} style={{display:"flex",gap:8,marginBottom:8}}><Avatar name={c.author} size={24}/><div style={{flex:1,background:G.bg3,borderRadius:10,padding:"7px 11px",border:`1px solid ${G.border}`}}><span style={{fontSize:11,fontWeight:700,color:"#fff",fontFamily:"'Syne',sans-serif"}}>{c.author.toLowerCase()} </span><span style={{fontSize:12,color:G.g600,fontFamily:"'Inter',sans-serif"}}>{c.text}</span></div></div>))}
...
function DMScreen({traders,onBack,openWithTrader}){
  ...
  if(conv&&ct)return(<div style={{display:"flex",flexDirection:"column",height:"100vh"}}>
    <div style={{display:"flex",alignItems:"center",gap:11,padding:"11px 16px",borderBottom:`1px solid ${G.border}`,background:G.bg2,borderTop:`1px solid ${G.border}`,flexShrink:0}}>
      ...
      <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#fff"}}>{ct.name.toLowerCase()}</div><div style={{fontSize:11,color:G.g400}}>{ct.category}</div></div>
    </div>
  ...
  return(<div>
    <div style={{padding:"13px 16px",borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",gap:10}}>
      ...
      <h1 style={{fontSize:20,fontWeight:800,color:"#fff",letterSpacing:"-0.03em"}}>Messages</h1>
    </div>
    ...
    {dms.map(d=>{const t=traders.find(x=>x.id===d.withId);if(!t)return null;return(<div key={d.id} onClick={()=>setActive(d)} className="hov" style={{display:"flex",alignItems:"center",gap:11,padding:"12px 16px",cursor:"pointer",borderBottom:`1px solid ${G.g50}`}}>
      ...
      <div style={{flex:1,minWidth:0}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span style={{fontSize:14,fontWeight:600,color:"#fff"}}>{t.name.toLowerCase()}</span><span style={{fontSize:11,color:G.g400}}>{timeAgo(d.lastTs)}</span></div><div style={{fontSize:13,color:d.unread>0?G.black:G.g400,fontWeight:d.unread>0?600:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.lastMsg||"Say hello!"}</div></div>
    </div>);})}
  </div>);
}
...
const PROP_FIRMS={
  ...
  csv:{name:"Generic CSV",color:"#1C1C1C",accent:"#00FF85",logo:"📁",hint:"Any CSV with a date, symbol, and pnl/profit/pl column.",detect:()=>true},
};
...
function PropVerifyView({currentUser,uploadState,setUploadState,myMetrics,setMyMetrics,myCategory,setMyCategory,submitVerification,onDonate,isGuest,onSignup}){
  ...
  function processFile(file){
    ...
    r.onload=e=>{
      try{
        ...
        const firm=detectFirm(rows);setDetectedFirm(firm);
        ...
        setMyMetrics(m);setUploadState("done");setStep("result");
      }catch(err){
  ...
  if(isGuest)return(<div style={{padding:20,background:G.black,minHeight:"100vh"}}><EmptyState emoji="🔒" title="Create an account first" subtitle="Verification ties your score to your identity." action={onSignup} actionLabel="Sign up free"/></div>);

  if(step==="result"&&myMetrics){
    const firm=PROP_FIRMS[detectedFirm||"csv"];const tier=getTier(myMetrics.traderScore);
    return(<div className="fade-up" style={{padding:18,background:G.black,minHeight:"100vh"}}>
      <div style={{background:firm.color,borderRadius:16,padding:"14px 16px",marginBottom:14,display:"flex",alignItems:"center",gap:11,border:`1px solid ${firm.accent}40`}}>
        ...
        <div style={{flex:1}}><div style={{fontSize:10,fontWeight:700,color:"#ffffff70",marginBottom:1,letterSpacing:"0.06em"}}>STATEMENT DETECTED</div><div style={{fontSize:15,fontWeight:700,color:"#fff"}}>{firm.name}</div><div style={{fontSize:11,color:"#ffffff60"}}>{parsedRows.length} trades analyzed</div></div>
        <div style={{background:"rgba(0,255,133,0.2)",borderRadius:20,padding:"4px 12px"}}><span style={{fontSize:11,fontWeight:700,color:G.green}}>✓ Verified</span></div>
      </div>
      <div style={{background:G.bg2,borderRadius:20,padding:20,marginBottom:14,border:`1px solid ${G.border}`}}>
        <div style={{fontSize:10,fontWeight:700,color:G.g500,marginBottom:12,letterSpacing:"0.07em"}}>YOUR TRADERSCORE</div>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
          <div style={{width:72,height:72,borderRadius:"50%",background:`conic-gradient(${tier.color} ${myMetrics.traderScore*3.6}deg,${G.bg3} 0deg)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 0 0 4px ${G.bg2},0 0 0 6px ${tier.color}40`}}>
            <div style={{width:56,height:56,borderRadius:"50%",background:G.bg2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:20,fontWeight:800,color:tier.color,lineHeight:1,fontFamily:"'JetBrains Mono',monospace"}}>{myMetrics.traderScore}</span><span style={{fontSize:9,fontWeight:600,color:tier.color,opacity:.8,letterSpacing:"0.04em"}}>{tier.name.toUpperCase()}</span></div>
          </div>
          <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
            {[["WIN RATE",myMetrics.winRate+"%",parseFloat(myMetrics.winRate)>50?G.profit:G.loss],["MAX DD",myMetrics.maxDrawdown+"%",parseFloat(myMetrics.maxDrawdown)<10?G.profit:G.loss],["PROF. FACTOR",myMetrics.profitFactor,"#fff"],["TRADES",myMetrics.tradeCount,"#fff"]].map(([k,v,c])=>(
              <div key={k} style={{background:G.bg3,borderRadius:10,padding:"8px 10px",border:`1px solid ${G.border}`}}><div style={{fontSize:8,fontWeight:700,color:G.g400,marginBottom:2,letterSpacing:"0.06em"}}>{k}</div><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:600,color:c}}>{v}</div></div>
            ))}
          </div>
        </div>
        {myMetrics.equityCurve?.length>2&&(()=>{const d=myMetrics.equityCurve;const mn=Math.min(...d),mx=Math.max(...d),rng=mx-mn||1,w=400,h=44;const pts=d.map((v,i)=>`${(i/(d.length-1))*w},${h-((v-mn)/rng)*h}`).join(" ");const isGreen=d[d.length-1]>=d[0];return(<div style={{borderRadius:10,overflow:"hidden",background:G.bg3,padding:"10px 0 0",border:`1px solid ${G.border}`}}><div style={{fontSize:9,fontWeight:700,color:G.g500,paddingLeft:12,marginBottom:4,letterSpacing:"0.06em"}}>EQUITY CURVE</div><svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{width:"100%",height:h,display:"block"}}><polyline points={pts} fill="none" stroke={isGreen?G.green:G.red} strokeWidth="2.5" strokeLinejoin="round"/></svg></div>);})()}
        <div style={{marginTop:12,padding:"9px 12px",background:G.greenLight,borderRadius:11,border:`1px solid ${G.greenBorder}`}}><div style={{fontSize:11,color:G.green,fontWeight:600,lineHeight:1.5}}>✓ Results from {firm.name} — cannot be manually edited or faked</div></div>
      </div>
      <div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:600,color:G.g500,marginBottom:8,letterSpacing:"0.06em",textTransform:"uppercase",fontFamily:"'Inter',sans-serif"}}>What do you primarily trade?</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{["Futures","Prop","Forex","Crypto","Stocks"].map(c=>(<button key={c} onClick={()=>setMyCategory(c)} style={{padding:"7px 14px",borderRadius:20,border:`1px solid ${myCategory===c?G.green:G.border}`,background:myCategory===c?G.greenLight:G.bg2,fontSize:12,fontWeight:600,color:myCategory===c?G.green:G.g500,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>{c}</button>))}</div></div>
      <Btn full size="lg" onClick={submitVerification} icon="✓">Claim my verified profile</Btn>
      <div style={{marginTop:10,textAlign:"center"}}><button onClick={()=>{setStep("choose");setUploadState("idle");setMyMetrics(null);}} style={{background:"none",border:"none",color:G.g400,cursor:"pointer",fontSize:12,fontFamily:"'Inter',sans-serif"}}>Upload a different statement</button></div>
    </div>);
  }

  if(step==="uploading")return(<div style={{padding:"60px 32px",textAlign:"center",background:G.black,minHeight:"100vh"}}>
    <div style={{fontSize:52,marginBottom:16}}>{selectedFirm?PROP_FIRMS[selectedFirm]?.logo:"📊"}</div>
    <div style={{fontSize:16,fontWeight:700,color:"#fff",marginBottom:8,fontFamily:"'Syne',sans-serif"}}>Analyzing your statement…</div>
    <div style={{fontSize:13,color:G.g500,marginBottom:24,fontFamily:"'Inter',sans-serif"}}>Reading trades, calculating metrics</div>
    <div style={{width:200,height:3,borderRadius:2,background:G.bg3,margin:"0 auto 20px",overflow:"hidden"}}><div style={{height:"100%",background:G.green,borderRadius:2,width:"60%"}}/></div>
    {uploadState==="error"&&<><div style={{background:"rgba(255,59,71,0.1)",border:"1px solid rgba(255,59,71,0.2)",borderRadius:12,padding:"12px 16px",color:G.red,fontSize:13,marginBottom:14,fontFamily:"'Inter',sans-serif"}}>{errMsg}</div><Btn variant="ghost" onClick={()=>setStep(selectedFirm?"instructions":"choose")}>← Try again</Btn></>}
  </div>);

  if(step==="instructions"&&selectedFirm){
    const firm=PROP_FIRMS[selectedFirm];
    const steps={ftmo:["Log in to your FTMO dashboard","Go to My Account → Trading History","Click 'Export CSV' top right","Upload that file below"],apex:["Open Tradovate (Apex trades live here)","Go to Activity → Performance tab","Select All Time date range","Click Export → upload here"],topstep:["Log in to TopstepX","Go to History → Trade History","Click Download CSV","Upload that file below"],mff:["Log in to MyFundedFutures","Go to Trading History","Click Export to CSV","Upload below"],tradovate:["Open Tradovate","Activity → Performance","All Trades → Export CSV","Upload below"],tradestation:["Open TradeStation","Reports → Trade Activity","Export CSV","Upload below"],csv:["Open your broker or platform","Find Trade History or Performance","Export or Download as CSV","Upload below — needs date, symbol, pnl columns"]};
    return(<div className="fade-up" style={{background:G.black,minHeight:"100vh"}}>
      <div style={{background:firm.color,padding:"18px 20px 20px",borderBottom:`1px solid ${firm.accent}30`}}>
        <button onClick={()=>setStep("choose")} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:20,padding:"6px 12px",color:"#fff",cursor:"pointer",fontSize:12,fontFamily:"'Inter',sans-serif",marginBottom:14,display:"flex",alignItems:"center",gap:5}}>← Back</button>
        <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:12}}>
          ...
          <div><div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:"'Syne',sans-serif"}}>{firm.name}</div><div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>Statement verification</div></div>
        </div>
        <div style={{background:"rgba(255,255,255,0.08)",borderRadius:12,padding:"11px 14px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:5,letterSpacing:"0.06em"}}>EXPORT GUIDE</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.8)",lineHeight:1.65,fontFamily:"'Inter',sans-serif"}}>{firm.hint}</div>
        </div>
      </div>
      <div style={{padding:"18px 18px 32px"}}>
        <div style={{fontSize:11,fontWeight:700,color:G.g500,marginBottom:12,letterSpacing:"0.06em"}}>STEPS</div>
        {(steps[selectedFirm]||steps.csv).map((s,i)=>(
          <div key={i} style={{display:"flex",gap:11,marginBottom:11,alignItems:"flex-start"}}>
            <div style={{width:24,height:24,borderRadius:"50%",background:firm.accent,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:11,fontWeight:700,color:"#000",marginTop:1}}>{i+1}</div>
            <div style={{fontSize:13,color:G.g600,lineHeight:1.55,flex:1,paddingTop:3,fontFamily:"'Inter',sans-serif"}}>{s}</div>
          </div>
        ))}
        {errMsg&&<div style={{background:"rgba(255,59,71,0.1)",border:"1px solid rgba(255,59,71,0.2)",borderRadius:12,padding:"10px 14px",color:G.red,fontSize:13,marginBottom:12,fontFamily:"'Inter',sans-serif"}}>{errMsg}</div>}
        <div onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={e=>{e.preventDefault();setDragOver(false);processFile(e.dataTransfer.files[0]);}} onClick={()=>localRef.current?.click()} style={{border:`2px dashed ${dragOver?firm.accent:G.border2}`,borderRadius:16,padding:"28px 20px",textAlign:"center",cursor:"pointer",background:dragOver?G.greenLight:G.bg2,marginBottom:14,transition:"all 0.2s"}}>
          <div style={{fontSize:32,marginBottom:8}}>📂</div>
          <div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:3,fontFamily:"'Syne',sans-serif"}}>Drop your CSV here</div>
          <div style={{fontSize:12,color:G.g500,fontFamily:"'Inter',sans-serif"}}>or tap to browse</div>
          <input ref={localRef} type="file" accept=".csv,.xlsx" style={{display:"none"}} onChange={e=>processFile(e.target.files[0])}/>
        </div>
        {["Your statement is private — only your score is shown publicly","We read trade data to calculate metrics — nothing stored raw","Official statements cannot be faked or edited"].map((t,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8,background:G.bg2,borderRadius:9,padding:"8px 12px",marginBottom:5,border:`1px solid ${G.border}`}}><span style={{fontSize:12,color:G.green}}>✓</span><span style={{fontSize:11,fontWeight:500,color:G.g500,fontFamily:"'Inter',sans-serif"}}>{t}</span></div>))}
      </div>
    </div>);
  }

  return(<div className="fade-up" style={{background:G.black,minHeight:"100vh"}}>
    <div style={{background:G.bg2,padding:"22px 20px 20px",borderBottom:`1px solid ${G.border}`}}>
      <div style={{fontSize:11,fontWeight:700,color:G.green,letterSpacing:"0.07em",marginBottom:6,fontFamily:"'Inter',sans-serif"}}>VERIFICATION</div>
      <div style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",lineHeight:1.2,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Where do you trade?</div>
      <p style={{fontSize:13,color:G.g500,lineHeight:1.55,fontFamily:"'Inter',sans-serif"}}>Select your prop firm or broker — we'll walk you through exactly how to export your statement.</p>
    </div>
    <div style={{background:G.greenLight,padding:"10px 20px",borderBottom:`1px solid ${G.greenBorder}`,display:"flex",gap:10,alignItems:"center"}}>
      <span style={{color:G.green,fontSize:13}}>✓</span><span style={{fontSize:12,color:G.green,fontWeight:600,fontFamily:"'Inter',sans-serif"}}>Statements are verified — results cannot be edited or faked</span>
    </div>
    <div style={{padding:"16px 16px 40px"}}>
      <div style={{fontSize:11,fontWeight:700,color:G.g500,marginBottom:10,letterSpacing:"0.06em",fontFamily:"'Inter',sans-serif"}}>PROP FIRMS</div>
      {FIRM_LIST.filter(f=>["ftmo","apex","topstep","mff"].includes(f.key)).map(firm=>(
        <button key={firm.key} onClick={()=>{setSelectedFirm(firm.key);setStep("instructions");}} style={{width:"100%",display:"flex",alignItems:"center",gap:13,padding:"14px 16px",marginBottom:9,borderRadius:16,border:`1px solid ${G.border}`,background:G.bg2,cursor:"pointer",textAlign:"left",transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=firm.accent;e.currentTarget.style.background=G.bg3;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=G.border;e.currentTarget.style.background=G.bg2;}}>
          <div style={{width:46,height:46,borderRadius:13,background:firm.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0,border:`1px solid ${firm.accent}30`}}>{firm.logo}</div>
          <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:2,fontFamily:"'Syne',sans-serif"}}>{firm.name}</div><div style={{fontSize:11,color:G.g500,fontFamily:"'Inter',sans-serif"}}>Statement export supported ✓</div></div>
          <div style={{fontSize:18,color:G.g400}}>›</div>
        </button>
      ))}
      <div style={{fontSize:11,fontWeight:700,color:G.g500,marginBottom:10,marginTop:18,letterSpacing:"0.06em",fontFamily:"'Inter',sans-serif"}}>FUTURES BROKERS</div>
      {FIRM_LIST.filter(f=>["tradovate","tradestation"].includes(f.key)).map(firm=>(
        <button key={firm.key} onClick={()=>{setSelectedFirm(firm.key);setStep("instructions");}} style={{width:"100%",display:"flex",alignItems:"center",gap:13,padding:"14px 16px",marginBottom:9,borderRadius:16,border:`1px solid ${G.border}`,background:G.bg2,cursor:"pointer",textAlign:"left",transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=firm.accent;e.currentTarget.style.background=G.bg3;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=G.border;e.currentTarget.style.background=G.bg2;}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:2,fontFamily:"'Syne',sans-serif"}}>{firm.name}</div><div style={{fontSize:11,color:G.g500,fontFamily:"'Inter',sans-serif"}}>CSV export supported ✓</div></div>
          <div style={{fontSize:18,color:G.g400}}>›</div>
        </button>
      ))}
      <div style={{fontSize:11,fontWeight:700,color:G.g500,marginBottom:10,marginTop:18,letterSpacing:"0.06em",fontFamily:"'Inter',sans-serif"}}>COMING SOON</div>
      {[{name:"Rithmic",logo:"⚙️",note:"Powers Apex, Topstep backend"},{name:"NinjaTrader",logo:"🥷",note:"Direct integration in progress"},{name:"Interactive Brokers",logo:"🏛️",note:"API connection coming"}].map(f=>(
        <div key={f.name} style={{display:"flex",alignItems:"center",gap:13,padding:"12px 16px",marginBottom:8,borderRadius:14,border:`1px solid ${G.border}`,background:G.bg2,opacity:0.5}}>
          <div style={{width:40,height:40,borderRadius:11,background:G.bg3,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{f.logo}</div>
          <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:G.g600,fontFamily:"'Syne',sans-serif"}}>{f.name}</div><div style={{fontSize:11,color:G.g400,fontFamily:"'Inter',sans-serif"}}>{f.note}</div></div>
          <span style={{fontSize:11,fontWeight:600,color:G.g400,background:G.bg3,borderRadius:20,padding:"3px 9px",fontFamily:"'Inter',sans-serif"}}>Soon</span>
        </div>
      ))}
      <div style={{marginTop:18,borderTop:`1px solid ${G.border}`,paddingTop:16}}>
        <div style={{fontSize:12,color:G.g500,textAlign:"center",marginBottom:10,fontFamily:"'Inter',sans-serif"}}>Don't see your broker?</div>
        <button onClick={()=>{setSelectedFirm("csv");setStep("instructions");}} style={{width:"100%",display:"flex",alignItems:"center",gap:13,padding:"13px 16px",borderRadius:14,border:`1.5px dashed ${G.border2}`,background:G.bg2,cursor:"pointer",textAlign:"left"}}>
          <div style={{width:40,height:40,borderRadius:11,background:G.bg3,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>📁</div>
          <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#fff",fontFamily:"'Syne',sans-serif"}}>Upload any CSV</div><div style={{fontSize:11,color:G.g500,fontFamily:"'Inter',sans-serif"}}>Needs date, symbol, and pnl columns</div></div>
          <div style={{fontSize:18,color:G.g400}}>›</div>
        </button>
      </div>
    </div>
...
function DonationModal({onClose,onSuccess}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"flex-end"}} onClick={onClose}><div className="slide-up" style={{background:G.bg2,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,margin:"0 auto",padding:"0 20px 36px",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
    <div style={{width:36,height:4,background:G.border3,borderRadius:2,margin:"14px auto 18px"}}/>
    {step==="choose"&&(<><div style={{textAlign:"center",marginBottom:22}}><div style={{width:58,height:58,borderRadius:"50%",background:`linear-gradient(135deg,${G.blue},#5AC8FA)`,margin:"0 auto 12px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,boxShadow:`0 6px 20px ${G.blue}50`}}>✓</div><h2 style={{fontSize:22,fontWeight:800,color:"#fff",marginBottom:6}}>Support TradeRank</h2><p style={{fontSize:13,color:G.g500,lineHeight:1.65,maxWidth:260,margin:"0 auto"}}>Any donation gets you a permanent blue checkmark. No subscription.</p></div>
...
function NotifPanel({notifs,onClose,onMarkAll}){
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:500}} onClick={onClose}><div className="fade-in" style={{position:"absolute",top:58,right:8,width:295,background:G.bg2,borderRadius:16,boxShadow:"0 8px 40px rgba(0,0,0,0.6)",border:`1px solid ${G.border}`,overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
    <div style={{padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${G.border}`}}><span style={{fontSize:15,fontWeight:800,color:"#fff"}}>Notifications</span><button onClick={onMarkAll} style={{background:"none",border:"none",color:G.green,cursor:"pointer",fontSize:12,fontWeight:700}}>Mark all read</button></div>
...
function JournalView(){
  ...
  return(<div>
    <div style={{padding:"16px 16px 0"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}><h1 style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.03em"}}>Journal</h1><button onClick={()=>openDay(today)} style={{background:G.green,border:"none",borderRadius:20,padding:"8px 16px",fontSize:13,fontWeight:600,color:"#fff",cursor:"pointer",boxShadow:`0 4px 12px ${G.green}50`}}>+ Log today</button></div>
    ...
    {logTab==="weekly"&&(<div style={{padding:"0 16px 24px"}}>
      ...
      {weeklyData.filter(d=>d.pnl!==null).slice().reverse().map(d=>{const entry=journal[d.date];const isPos=(d.pnl||0)>=0;const dispDate=new Date(d.date+"T12:00:00").toLocaleDateString("default",{weekday:"long",month:"short",day:"numeric"});return(<div key={d.date} onClick={()=>openDay(d.date)} className="hov" style={{display:"flex",alignItems:"center",gap:11,padding:"11px 2px",borderBottom:`1px solid ${G.g50}`,cursor:"pointer",borderRadius:10}}><div style={{width:42,height:42,borderRadius:12,background:isPos?G.greenLight:"#FFF0EF",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:16,fontWeight:900,color:isPos?G.profit:G.loss}}>{isPos?"↑":"↓"}</span></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:"#fff",marginBottom:1}}>{dispDate}</div>{entry?.note&&<div style={{fontSize:11,color:G.g500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{entry.note}</div>}</div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontFamily:"'DM Mono'",fontSize:14,fontWeight:600,color:isPos?G.profit:G.loss}}>{isPos?"+":"-"}${Math.abs(d.pnl).toLocaleString()}</div><div style={{fontSize:10,color:G.g400}}>{entry?.trades||0} trades</div></div></div>);})}
    </div>)}
    {logTab==="stats"&&(<div style={{padding:"0 16px 24px"}}>
      ...
      {tradingDays.length>0&&<div style={{background:G.g50,borderRadius:14,padding:14,border:`1px solid ${G.border}`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:600,color:"#fff"}}>Day win rate</span><span style={{fontSize:13,fontWeight:800,color:G.green}}>{Math.round((profitDays.length/tradingDays.length)*100)}%</span></div><div style={{height:8,borderRadius:4,background:G.g200,overflow:"hidden"}}><div style={{height:"100%",width:`${(profitDays.length/tradingDays.length)*100}%`,background:G.green,borderRadius:4}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:5}}><span style={{fontSize:11,color:G.profit,fontWeight:600}}>{profitDays.length} green</span><span style={{fontSize:11,color:G.loss,fontWeight:600}}>{lossDays.length} red</span></div></div>}
    </div>)}
  </div>);
}
  ...
  if(mode==="landing")return(
    <div style={{minHeight:"100vh",background:G.black,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 28px",textAlign:"center",position:"relative"}}>
        {/* Logo mark */}
        <div className="fade-up" style={{marginBottom:32,display:"flex",flexDirection:"column",alignItems:"center"}}>
          <div style={{width:72,height:72,background:G.green,borderRadius:20,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,boxShadow:`0 8px 40px rgba(0,255,133,0.25)`}}>
            <span style={{fontSize:26,fontWeight:800,color:"#000",fontFamily:"'Syne',sans-serif",letterSpacing:"-0.04em"}}>TR</span>
          </div>
          <div style={{fontSize:28,fontWeight:800,letterSpacing:"-0.04em",fontFamily:"'Syne',sans-serif",lineHeight:1}}>
            <span style={{color:"#fff"}}>trade</span><span style={{color:G.green}}>rank</span>
          </div>
        </div>
        <h1 className="fade-up" style={{animationDelay:"60ms",fontSize:"clamp(36px,9vw,56px)",fontWeight:800,color:"#fff",letterSpacing:"-0.04em",lineHeight:1.05,marginBottom:16,textAlign:"center",fontFamily:"'Syne',sans-serif"}}>
          Your performance,<br/><span style={{color:G.green}}>verified.</span>
        </h1>
        <p className="fade-up" style={{animationDelay:"120ms",fontSize:14,color:G.g500,lineHeight:1.7,marginBottom:40,maxWidth:280,textAlign:"center",fontFamily:"'Inter',sans-serif"}}>Verified rankings. Real trade data. Buy and sell proven strategies.</p>
        <div className="fade-up" style={{animationDelay:"180ms",display:"flex",flexDirection:"column",gap:10,width:"100%",maxWidth:320,alignItems:"center"}}>
          <Btn full size="lg" onClick={()=>setMode("signup")}>Create account — it's free</Btn>
          <button onClick={()=>setMode("login")} style={{width:"100%",padding:"14px",background:"transparent",border:`1px solid ${G.border2}`,borderRadius:100,fontSize:14,fontWeight:600,color:"#fff",cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Log in</button>
        </div>
        <button className="fade-up" style={{animationDelay:"240ms",marginTop:14,background:"none",border:"none",color:G.g500,cursor:"pointer",fontSize:12,fontFamily:"'Inter',sans-serif"}} onClick={()=>onAuth({id:0,name:"GUEST",email:"",category:"Futures",verified:false,supporter:false,traderScore:0,netReturn:"0.00",maxDrawdown:"0.0",winRate:"0.0",profitFactor:"—",tradeCount:0,equityCurve:[],followers:0,following:0,bio:"",isGuest:true})}>Browse as guest →</button>
      </div>
      {/* Stats bar */}
      <div style={{background:G.bg2,borderTop:`1px solid ${G.border}`,display:"flex",padding:"18px 0"}}>
        {[["1,200+","Traders"],["47K+","Trades verified"],["$12K+","Strategy sales"]].map(([n,l],i)=>(
          <div key={l} style={{flex:1,textAlign:"center",borderRight:i<2?`1px solid ${G.border}`:"none"}}>
            <div style={{fontSize:18,fontWeight:700,color:"#fff",fontFamily:"'Syne',sans-serif"}}>{n}</div>
  ...
  return(
    <div style={{minHeight:"100vh",background:G.black,maxWidth:480,margin:"0 auto",padding:"28px 24px",position:"relative"}}>
      ...
      <div style={{marginBottom:28}}>
        ...
        <h1 style={{fontSize:26,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",fontFamily:"'Syne',sans-serif",marginBottom:4}}>{isSignup?"Create account":"Welcome back"}</h1>
  ...
  return(<div style={{minHeight:"100vh",background:G.black,maxWidth:480,margin:"0 auto",position:"relative",fontSize:14,color:G.g900}}>
    ...
    {view==="discover"&&(<div style={{padding:"9px 16px",borderBottom:`1px solid ${G.border}`,position:"relative"}}>
      ...
      {searchFocus&&searchQuery.trim().length>1&&(<div className="fade-in" style={{position:"absolute",left:16,right:16,top:"calc(100% + 4px)",background:G.white,borderRadius:14,boxShadow:"0 8px 28px #00000018",border:`1px solid ${G.border}`,zIndex:300,overflow:"hidden"}}>
        {searchResults.length===0?<div style={{padding:"16px",fontSize:13,color:G.g400,textAlign:"center"}}>No traders found</div>
          :searchResults.slice(0,5).map(t=>(<div key={t.id} onClick={()=>{goProfile(t);setSearchQuery("");}} className="hov" style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",cursor:"pointer",borderBottom:`1px solid ${G.g50}`}}><Avatar name={t.name} size={32} verified={t.verified} supporter={t.supporter}/><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#fff"}}>{t.name.toLowerCase()}</div><div style={{fontSize:11,color:G.g400}}>{t.category}</div></div><ScorePill score={t.traderScore}/></div>))}
      </div>)}
    </div>)}
    ...
    <div style={{paddingBottom:80}}>
      ...
      {view==="discover"&&(<div className="fade-up">
        <div style={{padding:"12px 16px 7px"}}><h1 style={{fontSize:20,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",marginBottom:2}}>Discover</h1><p style={{fontSize:11,color:G.g500}}>Find and follow verified traders</p></div>
