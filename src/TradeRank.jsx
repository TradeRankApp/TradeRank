...
function AuthScreen({onAuth}){
  ...
  function signup(){if(!form.handle.trim())return setError("Handle is required.");if(form.handle.length<3)return setError("At least 3 characters.");if(!/^[A-Z0-9_]+$/i.test(form.handle))return setError("Letters, numbers, underscores only.");if(!form.email.includes("@"))return setError("Enter a valid email.");if(form.password.length<6)return setError("Password must be 6+ characters.");if(form.password!==form.confirm)return setError("Passwords don't match.");setLoading(true);setTimeout(()=>{const u={id:Date.now(),name:form.handle.toUpperCase(),email:form.email,category:form.category,verified:false,supporter:false,traderScore:0,netReturn:"0.00",maxDrawdown:"0.0",winRate:"0.0",profitFactor:"—",tradeCount:0,equityCurve:[],followers:0,following:0,bio:"",isMe:true,isNew:true};setLoading(false);onAuth(u);},800);}
  function login(){if(!form.email||!form.password)return setError("Fill in all fields.");setLoading(true);setTimeout(()=>{setLoading(false);const found=SEED_TRADERS.find(t=>t.name.toLowerCase()===form.email.toLowerCase()||t.email===form.email.toLowerCase());if(found){onAuth({...found,isMe:true});return;}setError("No account found. Try a trader name as email to demo.");},700);}
...
export default function TradeRank(){
  ...
  function doAuth(u){setCurrentUser(u);setLoggedIn(true);if(!u.isGuest&&!SEED_TRADERS.find(t=>t.id===u.id))setTraders(p=>[...p,u]);setView("feed");}
  ...
  function handlePost(data){setPosts(p=>[{id:Date.now(),traderId:currentUser.id,ts:Date.now(),...data,likes:0,comments:[],shares:0,liked:false,shared:false},...p]);setShowPost(false);}
  function handleFollow(tid){setFollowing(p=>{const n=new Set(p);n.has(tid)?n.delete(tid):n.add(tid);return n;});}
